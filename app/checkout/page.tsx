...
export default function CheckoutPage() {
  ...
  const discount = couponEligible === true ? calculateCouponDiscount(subtotal, coupon) : 0;
  ...
  function invalidateBenefit(messageText?: string) {
    ...
    setCompleted((current) => current.filter((value) => value !== 3 && value !== 4));
    if (step === 4) setStep(3);
  }
  ...
  function change(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (name === 'email' || name === 'phone') {
      invalidateBenefit('Confirme o cupom novamente após alterar seus dados.');
    }
    setForm((current) => ({
      ...current,
      [name]: name === 'zip'
  ...
  function nextFromCustomer() {
    if (!customerComplete) {
      setMessage('Preencha nome, e-mail e WhatsApp para continuar.');
      return;
    }
    setMessage(null);
    complete(1);
  ...
  function nextFromDelivery() {
    if (!deliveryComplete) {
      setMessage('Complete os dados de entrega para continuar. O CEP deve ter 8 dígitos.');
      return;
    }
    setMessage(null);
    complete(2);
  ...
  async function submit(event: FormEvent) {
    ...
    if (coupon.trim() && (!isFirstPurchaseCoupon(coupon) || couponEligible !== true)) {
      setStep(3);
    ...
    try {
      const response = await fetch('/api/create-checkout', {
        ...
        body: JSON.stringify({
          items,
          coupon: couponEligible === true ? normalizeCoupon(coupon) : '',
          shippingValue,
          ...
          {steps.map(({ number, label }) => {
            ...
            return (
              <button
                key={number}
                type="button"
                className={`checkout-progress-step${step === number ? ' is-active' : ''}${done ? ' is-complete' : ''}`}
                disabled={!canOpen(number)}
                aria-current={step === number ? 'step' : undefined}
                onClick={() => canOpen(number) && setStep(number)}
              >
        ...
        <div className="checkout-layout">
          <form onSubmit={submit} className="checkout-form-panel checkout-stepped-form" noValidate>
            <div className="checkout-first-purchase">
              ...
              <span>O benefício é validado pelo e-mail e telefone antes do pagamento.</span>
            </div>
            ...
            <section className={`checkout-stage${step === 2 ? ' is-active' : ''}${completed.includes(2) ? ' is-complete' : ''}`} aria-labelledby="checkout-address-title">
              ...
              {step === 2 ? <div className="checkout-stage-body">
                <div className="checkout-fields-address">
                  <div className="checkout-field"><label htmlFor="zip">CEP</label><input className="checkout-input" id="zip" required name="zip" inputMode="numeric" autoComplete="postal-code" value={form.zip} onChange={change} /></div>
                  <div className="checkout-field checkout-number"><label htmlFor="number">Número</label><input className="checkout-input" id="number" required name="number" value={form.number} onChange={change} /></div>
                </div>
                ...
                <div className="checkout-fields-address">
                  <div className="checkout-field"><label htmlFor="city">Cidade</label><input className="checkout-input" id="city" required name="city" autoComplete="address-level2" value={form.city} onChange={change} /></div>
                  <div className="checkout-field checkout-uf"><label htmlFor="state">UF</label><input className="checkout-input" id="state" required name="state" maxLength={2} autoComplete="address-level1" value={form.state} onChange={change} /></div>
                </div>
            ...
            <section className={`checkout-stage checkout-coupon-section${step === 3 ? ' is-active' : ''}${completed.includes(3) ? ' is-complete' : ''}`} aria-labelledby="checkout-coupon-title">
              ...
              {step === 3 ? <div className="checkout-stage-body">
                <p className="checkout-benefit-copy">Na primeira compra, use <strong>{FIRST_PURCHASE_COUPON}</strong>. A validação evita uso duplicado e acontece antes de liberar o pagamento.</p>
                <div className="checkout-coupon-row">
                  <input className="checkout-input" aria-label="Cupom de desconto" value={coupon} onChange={(event) => { setCoupon(event.target.value.toUpperCase().replace(/\s/g, '').slice(0, 20)); setCouponEligible(null); setCouponMessage(null); }} placeholder="Cupom" />
                  <button type="button" disabled={couponChecking} onClick={applyCoupon}>{couponChecking ? 'Validando…' : 'Aplicar'}</button>
                </div>
            ...
            <div className="checkout-summary-items">
              {lines.map(({ item, product }) => (
                <div key={item.productId} className="checkout-summary-item">
                  <div className="checkout-product-main"><div className="checkout-product-image"><Image src={product.images?.[0] || '/images/placeholder.svg'} alt={product.name} width={60} height={60} quality={82} /></div><div><span className="checkout-product-name">{product.name}</span><span className="checkout-product-price">{item.quantity} × {formatBRL(getEffectivePrice(product))}</span></div></div>
