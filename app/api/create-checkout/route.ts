...
const INFINITEPAY_HANDLE = 'ago-trancoso';
...
export async function POST(req: Request) {
  try {
    ...
    if (!response.ok || !data.url) {
      console.error('InfinitePay checkout error:', data);
      return NextResponse.json({ error: 'Não foi possível iniciar o pagamento pela InfinitePay.' }, { status: 502 });
    }
